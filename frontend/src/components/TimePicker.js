...
const TimePicker = ({id,label, value, onChange, maxDuration}) => {
    ...
    const [secondsOptions,setSecondsOptions] = useState(minutesSecondsOptions)
    ...
    const updateMinuteAndSecondOptions = useCallback((newHours,newMinutes) => {
        ...
        let newSecondsOptions = minutesSecondsOptions

        if(newHours === maxHours) {
            newMinuteOptions = Array.from({length: Math.max(0,maxMinutes)+1}, (_, i) => i)
            if(newMinutes === maxMinutes) {
                newSecondsOptions = Array.from({length: Math.max(0, maxSeconds)+1}, (_, i) => i)
            }
        }

        setMinuteOptions(newMinuteOptions)
        setSecondsOptions(newSecondsOptions)
            ...
            <div>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <Select
                        ...
                        options={secondsOptions.map(toOption)}
                        isSearchable
                    />
                </div>
            </div>
